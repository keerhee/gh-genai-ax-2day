# 공공기관 생성형 AI 트랙 — 기초 (session1)

출처: `PublicOffice_GenAI_Curriculum_System.md`. 단계 번호가 곧 수강 순서다.
각 단계의 산출물이 다음 단계의 입력이 된다. 중급 이후는 `../session2`에 있다.
...
같은 주제를 분량만 달리해 세 벌로 만들어 두었다. 상황에 맞는 하나만 쓴다.
...
`PublicOffice_GenAI_Excel_to_Claude_Appendix.html`은 축약본의 웹 산출물이다.
...
| 폴더 | 내용 |
|---|---|
| `PublicOffice_GenAI_Basic1_Docs_3H_data/` | 주간회의 메모 · 용어 브리프 2종 (실습 3·4·5) |
| `PublicOffice_GenAI_Basic2_Data_3H_data/` | 민원 794행 · 세그먼트 · 월별 · 프롬프트 템플릿 · 생성기 · 완성본 대시보드 |
| `PublicOffice_GenAI_Excel_to_Claude_Course_3H_data/` | 민원 794행 · 부서 마스터 · 월 요약 · what-if 파라미터 · 템플릿 |

기초 1의 실습 1·2·6은 첨부 없이 덱의 프롬프트만으로 진행한다.
완성본 웹앱은 `PublicOffice_GenAI_Basic2_Data_3H_data/answer/`에 있다.
이름에 `instructor`가 붙은 파일은 강사용이다. **수강생 배포 시 뺀다.**

원본 배포용 압축은 `PublicOffice_GenAI_Basic_3H_data.zip`과 `Excel_to_Claude_data.zip`으로 남겨 두었다.
